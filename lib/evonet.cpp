...
Evonet::Evonet(int nnetworks, int heterogeneous, int ninputs, int nhiddens, int noutputs, int nlayers, int nhiddens2, int bias, int netType, int actFunct, int outType, int wInit, int clip, int normalize, int randAct, double randActR, double wrange, int nbins, double low, double high)
{
  ...
  if (m_netType == 0)
    ...
    printf("neuro-modulated feed-forward ");
  if (m_bias)
    printf("with bias ");
  switch (m_actFunct)
    {
...
void Evonet::initNetArchitecture()
{
  ...
  *nbl = 0; nbl++;
  ...
  // neuromodulated feed-forward architecture, the first half of internal neurons are used to modulate the second half
  if (m_netType == 4)
    {
      // input->internal_regulatory_neurons connections	    
      *nbl = 0; nbl++;
      *nbl = m_ninputs; nbl++;
      *nbl = m_nhiddens / 2; nbl++;
      *nbl = m_ninputs-2; nbl++;
      *nbl = 2; nbl++;     // the block receive connections from the two inputs neurons only that encode the affordance vector
      m_nblocks++;
      // input->internal_regulated_neurons connections	    
      *nbl = 0; nbl++;
      *nbl = m_ninputs + (m_nhiddens / 2); nbl++;
      *nbl = m_nhiddens / 2; nbl++;
      *nbl = 0; nbl++; 
      *nbl = m_ninputs; nbl++; 
      m_nblocks++;
      // hidden update block
      *nbl = 3; nbl++;  // special update block for regulatory neurons
      *nbl = m_ninputs; nbl++;
      *nbl = m_nhiddens / 2; nbl++;
      *nbl = 0; nbl++;
      *nbl = 0; nbl++;
      m_nblocks++;
      // hidden update block
      *nbl = 4; nbl++;  // special update block for regulated neurons
      *nbl = m_ninputs + (m_nhiddens / 2); nbl++;
      *nbl = m_nhiddens / 2; nbl++;
      *nbl = 0; nbl++;
      *nbl = 0; nbl++;
      ...
      *nbl = m_ninputs + m_nhiddens; nbl++;
      *nbl = m_noutputs; nbl++;
      *nbl = m_ninputs + m_nhiddens / 2; nbl++; // the output layer receive only from the second half of internal neurons
      *nbl = m_nhiddens / 2; nbl++;
      m_nblocks++;
      // output update block
      *nbl = 1; nbl++;
  ...
  if (verbose == 1)
  {
    ...
    for (b = 0, nbl = m_netblock; b < m_nblocks; b++, nbl = (nbl + 5))
    {
      ...
      if (*nbl == 3)
        printf("neuron [%d-%d] updated with binary\n", *(nbl + 1), *(nbl + 1) + *(nbl + 2));
      if (*nbl == 4)
        printf("neuron [%d-%d] updated by neuromodulating\n", *(nbl + 1), *(nbl + 1) + *(nbl + 2));
    }
  }
      ...
      for (b = 0, nbl = m_netblock; b < m_nblocks; b++)
        {
          ...
          // update regulatory block, binary neurons
          if (*nbl == 3)
            {
              for(t = *(nbl + 1), a = (neurona + *(nbl + 1)), ni = (m_netinput + *(nbl + 1)), nt = (m_neurontype + *(nbl + 1)); t < (*(nbl + 1) + *(nbl + 2)); t++, a++, ni++, nt++)
                {
          ...
          // update regulated block, tanh neurons regulated by the preceeding block of neurons
          if (*nbl == 4)
            {
              for(t = *(nbl + 1), a = (neurona + *(nbl + 1)), ni = (m_netinput + *(nbl + 1)), nt = (m_neurontype + *(nbl + 1)); t < (*(nbl + 1) + *(nbl + 2)); t++, a++, ni++, nt++)
                {
