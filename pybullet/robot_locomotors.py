...
class WalkerBase(MJCFBasedRobot):
  ...
  def calc_state(self):
    ...
    more = np.array(
        ...
        dtype=np.float32)
    multibehavior = np.array([self.behavior1, self.behavior2], dtype=np.float32)
    return np.clip(np.concatenate([more] + [j] + [self.feet_contact] + [multibehavior]), -5, +5)
...
class Hopper(WalkerBase):
  ...
  def __init__(self):
    self.behavior1 = None
    self.behavior2 = None
    WalkerBase.__init__(self, "hopper.xml", "torso", action_dim=3, obs_dim=17, power=0.75)
